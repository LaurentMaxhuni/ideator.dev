    ...
    <div className="space-y-12">
      <section className="grid gap-8 lg:grid-cols-[1fr_auto] lg:items-end">
        <div>
          ...
          <h1 className="display-type mt-5 max-w-[700px] text-4xl font-normal leading-[0.98] text-foreground sm:text-6xl">Good to see you, {firstName}.</h1>
      ...
      <section>
        ...
        {projects.length === 0 ? (
          <div className="panel px-6 py-14 text-center sm:px-10">
            ...
            <p className="mx-auto mt-3 max-w-[440px] text-sm leading-6 text-muted-foreground">You do not need to arrive with an idea. Combine a person, a recurring tension, and a constraint to find a useful starting point.</p>
          ...
          <div className="grid gap-3">
            {projects.map((project, index) => (
              <Link key={project.id} href={`/app/projects/${project.id}`} className="group panel-quiet grid cursor-pointer gap-4 rounded-xl p-5 transition-[background-color,border-color,transform] duration-180 ease-spring hover:-translate-y-1 hover:border-accent/35 hover:bg-card/80 sm:grid-cols-[auto_1fr_auto] sm:items-center">
                <span className="tabular-nums text-[0.68rem] text-accent">{String(index + 1).padStart(2, "0")}</span>
                <div><div className="flex flex-wrap items-center gap-3"><h3 className="text-base font-medium text-foreground">{project.title}</h3><span className="rounded-sm border border-border px-2 py-1 text-[0.6rem] uppercase tracking-[0.1em] text-muted-foreground">{project.status}</span></div><p className="mt-2 text-xs text-muted-foreground">{project.domain} · updated {formatDate(project.updatedAt)}</p></div>
                <span className="inline-flex items-center gap-2 text-xs font-semibold text-primary sm:justify-self-end">Continue shaping <ArrowRight className="size-3.5 transition-transform duration-180 ease-spring group-hover:translate-x-1" /></span>
              </Link>
            ))}
          </div>
        )}
